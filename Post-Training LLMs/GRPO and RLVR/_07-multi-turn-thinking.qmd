...
The most dramatic demonstration of GRPO's potential came not from a carefully SFT'd model but from applying it directly to a *base model* that had never been fine-tuned on instructions. The DeepSeek-R1-Zero experiment challenged fundamental assumptions about what RL can and cannot teach LLMs.

### R1-Zero: RL Directly on a Base Model

DeepSeek-R1-Zero applied GRPO with rule-based rewards (math verification) to DeepSeek-V3-Base, a 685-billion-parameter base model. The setup was remarkably simple: the prompt template wrapped each math problem with instructions to reason inside `<think>` tags and produce a final answer inside `<answer>` tags. No SFT data was used. No examples of correct reasoning were shown.
...
These emergent behaviors were collectively called the "Aha moment" in the DeepSeek-R1 paper ([Guo et al., 2025](https://arxiv.org/abs/2501.12948)), and they captured the imagination of the AI research community. The idea that pure RL, without any demonstration data, could induce sophisticated reasoning behaviors seemed to validate the promise of scalable AI alignment.
...
::: {.predict-reveal}
**The answer is B.** The Dr. GRPO team found that self-reflection keywords ("Aha," "Wait," "let me verify") already appear in base model outputs before any RL training. These patterns exist in web text that the models were pretrained on. RL does not create the behavior from scratch; it increases its frequency and (sometimes) its usefulness. This reframes the R1-Zero narrative: RL is a refinement tool that amplifies pre-existing capabilities, not a creation tool that produces entirely new reasoning patterns.
:::
...
The Dr. GRPO paper ([Liu et al., 2025](https://arxiv.org/abs/2503.20783)) cast important doubt on this narrative. By testing multiple base models *before* RL training, they found that:
...
3. **Some base models already solve math problems without any template.** Qwen2.5-Math models, when given questions without any instruction template, achieved 60% higher accuracy than with the standard R1 template. This accuracy gap suggests these models were pretrained on concatenated question-answer pairs, essentially receiving SFT through pretraining.
...
DeepSeek-R1 (the production model, not R1-Zero) used a sophisticated four-stage pipeline that combined pure RL with SFT in a carefully orchestrated sequence:

![The DeepSeek-R1 training pipeline. Stage 1 applies cold-start SFT, Stage 2 uses RL with reasoning rewards, Stage 3 performs rejection sampling to create new SFT data, and Stage 4 applies final RL for alignment. Source: Guo et al. (2025).](GRPO and RLVR/images/r1-pipeline.png){#fig-r1-pipeline}
...
2. **Reasoning-oriented RL**: GRPO with rule-based rewards on math, coding, and logic tasks. This stage is where the model's reasoning capabilities improved most dramatically. The reward functions were task-specific verifiers (math answer checking, code test execution).

3. **Rejection sampling SFT**: After RL, the model's best outputs (from the RL-trained policy) were used as new SFT data for a second round of supervised training. This distillation step captures the RL improvements in a cleaner model that is more stable and less prone to RL artifacts like excessive hedging or irrelevant self-reflection.
...
Multi-turn GRPO training is conceptually straightforward but practically complex. The "output" is no longer a single SQL query but an entire conversation trajectory. The reward may apply only to the final outcome of the conversation, requiring credit assignment across multiple turns. Group sampling now generates $G$ complete trajectories per initial prompt, which is more expensive than single-turn sampling.
...
::: {.feedback-incorrect}
Not quite. The key issue is credit assignment. In multi-turn GRPO, the reward applies to the entire conversation trajectory, and the group-relative advantage is the same for all tokens across all turns. The model cannot distinguish which turn's edits actually drove the improvement. First-turn queries get better because they directly affect the reward; refinement turns get a diluted signal because their contribution depends on the full interaction history. This is distinct from trajectory length (a memory problem) or environment statefulness (a parallelization problem).
:::

:::

These challenges are active research areas. For Ava, the practical recommendation is to start with single-turn GRPO (simpler and faster), validate that it improves performance, and extend to multi-turn only if the task requires it.
