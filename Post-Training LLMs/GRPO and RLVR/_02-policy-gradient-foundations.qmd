...
This is the **high variance problem** of Monte Carlo policy gradients (see @sec-math-expectation-variance for a review of variance and the sample mean). With binary rewards, the gradient is either the full log-probability gradient (reward 1) or zero (reward 0). The signal is extremely noisy, and training oscillates instead of converging. For Ava's model, after thousands of updates, accuracy barely improves because each update is as likely to help as to hurt; the gradient estimates simply do not agree with each other.
