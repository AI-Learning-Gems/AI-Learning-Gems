...
**Environment note:** This workflow is designed for agentic execution in Cursor, Antigravity, Windsurf, or similar coding-agent IDEs. Tool names may vary by platform:
- **Read a file:** `Read` (Cursor) / `view_file` (Antigravity) / `read_file`
- **Run shell command:** `Shell` (Cursor) / `run_terminal_command` (Antigravity)
- **Spawn subagent:** `Task` (Cursor) / `create_subagent` + `run_subagent` (Antigravity)
- **Web search (DISABLED for main agent):** `WebSearch` / `search_web`
- **Web fetch (DISABLED for main agent):** `WebFetch` / `read_url_content`

Use whichever tool names your platform provides. The architecture is the same regardless of tool naming.
...
│                                                                         │
│ TOOLS AVAILABLE: Read/view_file, Shell/run_terminal_command, Write          │
│ TOOLS DISABLED:  WebFetch/read_url_content, WebSearch/search_web            │
│                                                                         │
...
**Immediately after reading the user's query**, spawn a subagent (`Task` in Cursor, `create_subagent`/`run_subagent` in Antigravity) with the following prompt. Do NOT do any web searching yourself. Do NOT use `WebFetch`/`read_url_content` or `WebSearch`/`search_web`. Your only job before spawning the subagent is to understand the query well enough to pass it along.
